...
func TestImagesToTransfer_RoundTrip(t *testing.T) {
	...
	elm := toXMLImagesToTransfer(orig, NsWSCN+":ImagesToTransfer")

	if elm.Name != NsWSCN+":ImagesToTransfer" {
		t.Errorf("expected element name '%s', got '%s'",
			NsWSCN+":ImagesToTransfer", elm.Name)
	}
	if elm.Text != "10" {
...
func TestImagesToTransfer_NoAttributes(t *testing.T) {
	orig := ImagesToTransfer{
		Value: 5,
	}
	...
	if elm.Text != "5" {
		t.Errorf("expected text '5', got '%s'", elm.Text)
	}
...
func TestImagesToTransfer_Validation(t *testing.T) {
	tests := []struct {
		name    string
		text    string
		wantErr bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			decoded, err := decodeImagesToTransfer(root)
			if tt.wantErr {
				if err == nil {
...
func TestImagesToTransfer_FromXML(t *testing.T) {
	...
	if decoded.Value != 25 {
		t.Errorf("expected value 25, got %v", decoded.Value)
	}
...
func TestImagesToTransfer_EdgeCases(t *testing.T) {
	tests := []struct {
		name  string
		value int
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			decoded, err := decodeImagesToTransfer(elm)
			if err != nil {
				t.Fatalf("decode returned error for value %d: %v", tt.value, err)
			}
